function [hhh,hTxt]=vline(x,in1,in2)
...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        if xunit<0.8
            if labelAtTop
                hTxt = text(x+0.01*xrange,y(2)-0.1*(y(2)-y(1)),label,'color',get(h,'color'));
            else
                hTxt = text(x+0.01*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
            end
        else
            if labelAtTop
                hTxt = text(x-.05*xrange,y(2)-0.1*(y(2)-y(1)),label,'color',get(h,'color'));
            else
                hTxt = text(x-.05*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'));
            end
        end
    end     
