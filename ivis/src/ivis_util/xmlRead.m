...
% DESCRIPTION
%
% PETE: post-2007a try..catch syntax removed because 2007a is a load of rubbish and can't handle it.
%
...
%
%
%   Pete - Added logicals (line ~438)
%   PJ [30/01/13] - added logical arrays
...
if (LeafNode)
  ...
  if (LeafNode>0) % supported leaf node types
      ...
      catch ME         %#ok<CTCH> catch for mablab versions prior to 7.5
rethrow(ME)          
...
% Can this string 'str' be converted to a logical? if so than do it. [PETE]
if ischar(val)
    if strcmpi(val,'true')
        val = true;
    elseif strcmpi(val,'false')
        ...
        % might this be an array of logicals?
        % should accept the forms: [true, false, false]; [true false true];
        % [true,  false,  true]
        tmp = regexp(val,',*\w+','match');
        if all(ismember(tmp,{'true','false'}))
            val = strcmpi(tmp,'true'); % convert
        end
    end
end

% further support for logicals, logical arrays, and cell arrays [PETE]
if ischar(val)
    % Might this be a logical? if so then convert
    if strcmpi(val,'true')
        val = true;
    elseif strcmpi(val,'false')
        val = false;
    elseif regexp(val, '^\[.+\]$') % if inside square bracket '^\[\s*[\w\s,]+\s*\]$')
        val = regexprep(val, '^\[(\s)*', ''); % remove any leading bracket
        val = regexprep(val, '(\s)*\]$', ''); % remove any trailing bracket
        val = regexp(val, '[\s,]+', 'split'); % split into cell array
        % convert to array of logicals if required
        if all(ismember(val,{'true','false'}))
            val = strcmpi(val,'true'); % convert
        end
    end
end
