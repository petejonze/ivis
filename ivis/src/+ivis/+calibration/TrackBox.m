classdef (Sealed) TrackBox < Singleton & ivis.broadcaster.IvListener
    ...
    %
    % @ todo: many values are currently hard coded ? (e.g., screen
    % dimensions) 
    %
    %
    ...
    methods (Access = public)
        ...
        function obj = TrackBox()
            ...
            % set geometry
            obj.boxdist_mm = obj.eyeTracker.PREFERRED_VIEWING_DISTANCE_MM;
            obj.boxsize_mm = obj.eyeTracker.TRACKBOX_SIZE_MM;
            obj.trackerInFrontMonitor_mm = obj.eyeTracker.trackerInFrontMonitor_mm;
            
            % set other internal params
            ...
            zmax = obj.trackerInFrontMonitor_mm+obj.boxdist_mm+obj.boxsize_mm/2;
        ...
        function [] = draw(obj,~,~)
            ...
            if ~ismember(obj.eyeTracker.NAME, {ivis.eyetracker.IvTobii.NAME, ivis.eyetracker.IvTobiiEyeX.NAME})
                fprintf('TrackBox only defined currently for Tobii and TobiiEyeX. Disabling\n');
                obj.stopListening('PreFlip');
                return
            end
            ...
            % point
            leyeposition = obj.ucs2mycs(obj.eyeTracker.lEyeball_xyzt_raw(1:3), 0, 0, obj.trackerInFrontMonitor_mm);
            reyeposition = obj.ucs2mycs(obj.eyeTracker.rEyeball_xyzt_raw(1:3), 0, 0, obj.trackerInFrontMonitor_mm);
            ...
            if (any(isnan(leyeposition)) || all(leyeposition == [0 0 -150])) ...
                    && (any(isnan(reyeposition)) || all(reyeposition == [0 0 -150])) % draw red if not tracking
            ...
            glTranslated(obj.normalise(obj.monitorwidth_mm/2), 0, obj.normalise(obj.trackerInFrontMonitor_mm+obj.boxdist_mm) );
        ...
        function [] = start(obj)
            ...
            % @author   PRJ
            %  
            fprintf('Starting trackbox listener. Remember, this will only display if you have called ''import ivis.broadcaster.*'' in your main script, prior to any Scree(''Flip'') calls.\n');
