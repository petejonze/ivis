...
%     IvMain.initialise(IvParams.getDefaultConfig('GUI.useGUI',false, 'calibration.GUIidx',2));
...
%         targs = calib.getTargs(n, w, h);
%         for i = 1:n
%             while ~any(InH.getInput() == InH.INPT_SPACE.code)
%                 eyetracker.refresh(); % logging enabled
%                 Screen('DrawDots', winhandle, targs(i,:), 40, [255 0 0], [], 1);
%                 Screen('Flip', winhandle);
%                 WaitSecs(1/60);
%             end
%             resp = log.data.getLastN(10, 1:2);
%             calib.addMeasurements(targs(i,:), resp);
%         end
%         
...
%         while ~any(InH.getInput() == InH.INPT_SPACE.code)
%             eyetracker.refresh(false); % false to suppress data logging
%             DrawFormattedText(winhandle, 'press space to end');
%             Screen('Flip', winhandle);
%             WaitSecs(1/60);
%         end
% 
